...
class PointProcessUpdateConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.map_step_size < 0.0:
            raise ValueError("map_step_size must be non-negative")
        ...
        if not 0.0 < self.covariance_damping <= 1.0:
            raise ValueError("covariance_damping must be in (0, 1]")
        if self.max_state_update_norm <= 0.0:
            raise ValueError("max_state_update_norm must be positive")
...
def normal_flow_activities(
    ...
    if velocity_norm <= 1e-12:
        return np.zeros(normals.shape[0], dtype=float)
