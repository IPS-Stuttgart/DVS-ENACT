...
class ProjectedOutputRefiner:
    ...
    def __init__(
        ...
        *,
        ...
        validate_projection_motion_smoothing(projection_motion_smoothing)
        ...
        validate_projection_size_clamp(projection_size_clamp_ratio)
        ...
        self.projection_motion_smoothing = projection_motion_smoothing
        ...
        self.projection_size_clamp_ratio = projection_size_clamp_ratio
        ...
        self._previous_accepted_candidate_center: np.ndarray | None = None
    ...
    def reset_state(self) -> None:
        ...
        self._previous_accepted_candidate_center = None
    ...
    def refine(self, candidate_bbox: Any, events: Any, **kwargs: Any) -> DVSRefinementResult:
        ...
        unclipped_projected_xywh = project_refinement_output(
            ...
            previous_candidate_center=self._previous_accepted_candidate_center,
            ...
            projection_motion_smoothing=self.projection_motion_smoothing,
            ...
            projection_size_clamp_ratio=self.projection_size_clamp_ratio,
    ...
    def observe_refinement_decision(
        ...
        """Record accepted projected center/size for the next frame."""
        if not accepted or result.fallback_reason is not None:
            return
        accepted_xywh = np.asarray(result.as_xywh(), dtype=float)
        if (
            (
            ...
            self._previous_accepted_projected_center = (
                accepted_xywh[:2] + 0.5 * accepted_xywh[2:]
            ).copy()
            self._previous_accepted_candidate_center = (
                candidate_xywh[:2] + 0.5 * candidate_xywh[2:]
            ).copy()
        if (
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Optional temporal center smoothing for projected outputs. The value "
            "is the weight of the previous accepted projected center; 0 uses "
            "the current projection and 1 holds the previous center."
        ),
    )
    parser.add_argument(
        ...
        type=float,
        help=(
            "Optional motion-compensated center smoothing for projected outputs. "
            "The value is the weight of the previous accepted projected center "
            "advanced by the base tracker's frame-to-frame motion."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Projected center shifts larger than this ratio are capped."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Projected size changes larger than this ratio are capped."
        ),
    )
    parser.add_argument(
...
def run_from_args(args: argparse.Namespace) -> dict[str, Any]:
    ...
    refiner = ProjectedOutputRefiner(
        ...
        projection_motion_smoothing=args.projection_motion_smoothing,
        ...
        projection_size_clamp_ratio=args.projection_size_clamp_ratio,
...
def project_refinement_output(
    ...
    *,
    ...
    previous_candidate_center: np.ndarray | None = None,
    ...
    output = smooth_projected_center_by_motion(
        candidate,
        output,
        refinement_mode=refinement_mode,
        previous_projected_center=previous_projected_center,
        previous_candidate_center=previous_candidate_center,
        projection_motion_smoothing=projection_motion_smoothing,
    )
    ...
    output = apply_projection_center_clamp(
        candidate,
        output,
        projection_center_clamp_ratio=projection_center_clamp_ratio,
    )
    output = apply_projection_size_clamp(
        candidate,
        output,
        refinement_mode=refinement_mode,
        projection_size_clamp_ratio=projection_size_clamp_ratio,
    )
...
def apply_projection_center_clamp(
    ...
    *,
    projection_center_clamp_ratio: float | None,
) -> np.ndarray:
    """Cap projected center shifts relative to the base-box diagonal."""
    validate_projection_center_clamp(projection_center_clamp_ratio)
    output = np.asarray(projected_xywh, dtype=float).reshape(4).copy()
    if projection_center_clamp_ratio is None:
        return output
    ...
    if diagonal <= 1e-9:
        return output
    shift = output_center - candidate_center
    shift_norm = float(np.linalg.norm(shift))
    max_shift = float(projection_center_clamp_ratio) * diagonal
    if shift_norm <= max_shift or shift_norm <= 1e-9:
        return output
    clamped_center = candidate_center + shift * (max_shift / shift_norm)
    output[:2] = clamped_center - 0.5 * output[2:]
    return output


def apply_projection_size_clamp(
    candidate_xywh: np.ndarray,
    projected_xywh: np.ndarray,
    *,
    refinement_mode: str,
    projection_size_clamp_ratio: float | None,
) -> np.ndarray:
    """Cap projected size changes relative to base width/height."""
    validate_refinement_mode(refinement_mode)
    validate_projection_size_clamp(projection_size_clamp_ratio)
    output = np.asarray(projected_xywh, dtype=float).reshape(4).copy()
    if projection_size_clamp_ratio is None:
        return output
    ...
    size_delta = output[2:] - candidate[2:]
    max_delta = float(projection_size_clamp_ratio) * candidate_size
    clamped_size = np.array(output[2:], dtype=float, copy=True)
    clamped_size[axes] = candidate[2:][axes] + np.clip(
        size_delta[axes],
        -max_delta[axes],
        max_delta[axes],
    )
    ...
    output[:2] = center - 0.5 * clamped_size
    output[2:] = clamped_size
    return output
...
def projection_confidence_value(result: Any, field: str | None) -> float | None:
    ...
    if field == "active_fraction":
        return active_measurement_fraction(result)
    if field == "event_support_score":
        return event_support_score(result)
...
def event_support_score(result: Any) -> float:
    """Return a conservative [0, 1] confidence score from DVS diagnostics."""
    activity = finite_unit_interval(
        getattr(result, "mean_event_activity", None),
        default=0.0,
    )
    active_fraction = active_measurement_fraction(result) or 0.0
    event_count_strength = used_event_count_strength(result)
    components = [activity, active_fraction, event_count_strength]

    polarity_consistency = finite_unit_interval(
        getattr(result, "polarity_consistency_fraction", None),
        default=None,
    )
...
def active_measurement_fraction(result: Any) -> float | None:
...
def used_event_count_strength(result: Any) -> float:
...
def smooth_projected_center(
    ...
    *,
    ...
    smoothed_center = (1.0 - smoothing) * current_center + smoothing * previous_center
    output[:2] = smoothed_center - 0.5 * output[2:]
    return output


def smooth_projected_center_by_motion(
    candidate_xywh: np.ndarray,
    projected_xywh: np.ndarray,
    *,
    refinement_mode: str,
    previous_projected_center: np.ndarray | None,
    previous_candidate_center: np.ndarray | None,
    projection_motion_smoothing: float | None,
) -> np.ndarray:
    """Blend projected center toward previous accepted correction plus base motion."""
    validate_refinement_mode(refinement_mode)
    validate_projection_motion_smoothing(projection_motion_smoothing)
    output = np.asarray(projected_xywh, dtype=float).reshape(4).copy()
    if (
    ...
    candidate = np.asarray(candidate_xywh, dtype=float).reshape(4)
    current_candidate_center = candidate[:2] + 0.5 * candidate[2:]
    previous_center = np.asarray(previous_projected_center, dtype=float).reshape(2)
    previous_candidate = np.asarray(previous_candidate_center, dtype=float).reshape(2)
    predicted_center = previous_center + current_candidate_center - previous_candidate
    current_center = output[:2] + 0.5 * output[2:]
    smoothing = float(projection_motion_smoothing)
    smoothed_center = (1.0 - smoothing) * current_center + smoothing * predicted_center
    output[:2] = smoothed_center - 0.5 * output[2:]
    return output
...
def validate_projection_motion_smoothing(
    projection_motion_smoothing: float | None,
) -> None:
    """Raise ValueError for invalid motion-compensated smoothing factors."""
    if projection_motion_smoothing is None:
        return
    if not 0.0 <= float(projection_motion_smoothing) <= 1.0:
...
def validate_projection_center_clamp(
    ...
    """Raise ValueError for invalid projection center clamp ratios."""
    if projection_center_clamp_ratio is None:
        return
    if float(projection_center_clamp_ratio) < 0.0:
...
def validate_projection_size_clamp(
    projection_size_clamp_ratio: float | None,
) -> None:
    """Raise ValueError for invalid projection size clamp ratios."""
    if projection_size_clamp_ratio is None:
        return
    if float(projection_size_clamp_ratio) < 0.0:
